...
Most mid-size companies can't afford that. But "so we'll be the cheap
one" is the wrong conclusion, and it's worth being precise about why
— a 2026-09-16 market scan (`RESEARCH.md`) found the cheap slot is
already taken by *free*: CrowdSec, SafeLine and Coraza are open-source
and self-hosted, and Cloudflare has a free tier. Nobody wins a price
...
proposing a change of direction — it likely already says why.
