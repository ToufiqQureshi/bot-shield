# PROGRESS archive (sessions up to 2026-09-22)

Long-form session records from before `PROGRESS.md` became an index.

Nothing here is wrong; it is simply verbose, and a session that reads the
whole file spends its context on history rather than on the code. Kept
because it contains reasoning, rejected approaches and verification detail
that the diffs alone do not carry.

**Read this only when the commit message and the topic documents do not
answer the question.** Start from `docs/PROGRESS.md`.
