...
// same limitation as Stats, and durable history needs the planned
...
// Handler serves the evidence endpoint for the dashboard. Unlike the
// aggregate stats endpoint this returns per-visitor fingerprints and
// would tell a caller whether their own fingerprint is being flagged,
// so it needs the token and gets no wildcard CORS.
func (t *Trail) Handler(token string) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewEncoder(w).Encode(t.Recent(limit))
	})
}

// authorized checks the bearer token. An unset token denies everyone,
// so a deployment that forgot to configure one fails closed instead of
// publishing visitor records.
